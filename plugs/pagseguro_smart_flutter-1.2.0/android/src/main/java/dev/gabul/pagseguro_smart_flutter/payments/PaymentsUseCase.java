...
public class PaymentsUseCase {
  ...
  private Observable<ActionResult> doPayment(
    ...
    return Observable.create(emitter -> {
      ...
      // customDialog.setMaxTimeShowPopup(1); //! reduce to 1.
      // mPlugPag.setPlugPagCustomPrinterLayout(customDialog);
        mPlugPag.setPrintActionListener(new PlugPagPrintActionListener() {
        @Override
        public void onError(PlugPagException exception) {
            // Tratar erro na impressão/envio de comprovante
            Log.e("PlugPag", "Erro: " + (exception != null ? exception.getMessage() : "Erro desconhecido"));
        }
        ...
        public void onPrint(String phoneNumber, PlugPagTransactionResult transactionResult, PlugPagPrintActionListener.OnFinishPlugPagPrintActions onFinishActions) {
            if (onFinishActions != null && mPlugPag != null) {
                onFinishActions.doNothing(mPlugPag); // DISPENSA tudo
                Log.d("PlugPag", "Dispensada impressão/envio");
            }
        }
    });
  ...
  public void beep() {
    try {
      PlugPagBeepData beepData = new PlugPagBeepData(
              PlugPagBeepData.FREQUENCE_LEVEL_2, 200);
