...
enum StatusTransactionEnum {
  ...
  ///! spolus empty
  EMPTY_RESPONSE("Ação não encontrada"), //! program a function to send a call to server informing we got an unkown response

  ///! spolus waiting
  WAITING("Aguarde..."),

  ///! spolus qrcode
  TRANSACTION_WAITING_QRCODE_SCAN("Aponte a câmera ao código acima");

  // ///! spolus function fail
  // ERROR("Falha no pagamento");

  /// Values of the enum
  final String descriptiom;

  const StatusTransactionEnum(this.descriptiom);
  ...
  static StatusTransactionEnum fromName(String name) {
    ...
    if (listValues.isEmpty) {
      var emptyReturn = StatusTransactionEnum.EMPTY_RESPONSE;
      return emptyReturn;
    } else {
